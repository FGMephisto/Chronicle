...
function getAbilityScore(rActor, sAbility, rEffect)
	...
	elseif StringManager.contains(DataCommon.classes, sAbility:lower()) then
		...
		local sTech = ActionsManager2.ConvertToTechnical(sAbility);
		nStatScore = DB.getValue(nodeActor, "abilities." .. sTech .. ".score", 0);
		if nStatScore == 0 then
			nStatScore = DB.getValue(nodeActor, "abilities." .. sAbility:lower() .. ".score", 0);
		end
	end
...
function getDefenseValue(rAttacker, rDefender, rRoll)
	...
	elseif ActorManager.isRecordType(rDefender, "npc") or ActorManager.isRecordType(rDefender, "vehicle") then
		if (rRoll.sSubtargetPath or "") ~= "" then
			nDefense = DB.getValue(DB.getPath(rRoll.sSubtargetPath, "defenses.ac.total"), 0);
			if nDefense == 0 then
...
--
--	DAMAGE VULN / RESIST / IMMUNE HELPERS
--
...
function getArmorPenalty(rActor)
	...
	return DB.getValue(nodeActor, "defenses.armor.penalty", 0);
end
...
function getSkillRank(rActor, sSkill)
	...
	return DB.getValue(nodeSkill, "misc", 0);
end