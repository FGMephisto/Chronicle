...
function onVehiclePostAdd(tCustom)
	...
	for _,v in pairs(DB.getChildren(tCustom.nodeCT, "traits")) do
		CombatManager2.parseNPCPower(rActor, v, aEffects)
	end
	for _,vComponent in pairs(DB.getChildren(tCustom.nodeCT, "components")) do
		DB.setValue(vComponent, "locked", "number", 1)
		
		for _,v in pairs(DB.getChildren(vComponent, "actions")) do
			CombatManager2.parseNPCPower(rActor, v, aEffects)
...
function parseAttackLine(sLine)
	-- Debug.chat("FN: parseAttackLine in manager_combat2")
	-- ToDo: Adjust this
	local rPower = nil
	...
	if nIntroStart then
		...
		while nAbilityStart do
			...
			elseif sAbility:sub(1,4) == "ATK:" and #sAbility > 4 then
				...
				rAttack.nStat = tonumber(sAbility:sub(6,7)) or 1
				rAttack.nSkill = tonumber(sAbility:sub(9,10)) or 0
				rAttack.nMod = tonumber(sAbility:sub(12,13)) or 0
...
function rollEntryInit(nodeEntry)
	...
	-- Get last initiative
	local nInitResult = DB.getValue(nodeEntry, "initresult", "number", 0)

	-- Get actor
	local rActor = ActorManager.resolveActor(nodeEntry)
	local rRoll = {}

	-- Get sheet class (PC/NPC) and class record (#1, #2 ...)
	local sClass, sRecord = DB.getValue(nodeEntry, "link", "", "")

	-- For PCs, we always roll unique initiative
	if sClass == "charsheet" then
		ActionInit.performRoll(draginfo, rActor, true, true)
		return
	end
...
-- ===================================================================================================================
-- Determine which CT entries should roll initiative
-- ===================================================================================================================
