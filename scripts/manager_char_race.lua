...
function getRaceSubraceOptions(sRaceName)
	local tOptions = {};
...
function getRaceFromSubrace(sSubraceName)
	...
	for _,sMapping in ipairs(tMappings) do
		for _,vRace in ipairs(DB.getChildrenGlobal(sMapping)) do
			for _,vSubRace in ipairs(DB.getChildList(vRace, "subraces")) do
				local sMatch = StringManager.trim(DB.getValue(vSubRace, "name", "")):lower();
...
function helperAddRaceSubraceChoice(rAdd)
	local tRaceSubraceOptions = CharRaceManager.getRaceSubraceOptions(rAdd.sSourceName);
	...
	if #tRaceSubraceOptions == 1 then
		...
		rAdd.sSubraceChoice = tRaceSubraceOptions[1].text;
		CharRaceManager.helperAddRaceSubrace(rAdd);
	else
...
function callbackAddRaceSubraceChoice(tSelection, rAdd)
	...
	rAdd.sSubraceChoice = tSelection[1];
	CharRaceManager.helperAddRaceSubrace(rAdd);
end
function helperAddRaceSubrace(rAdd)
	if ((rAdd.sSubraceChoice or "") == "") then
		return;
	end

	-- Get subrace data path from name
	local sSubracePath = nil;
	local tRaceSubraceOptions = CharRaceManager.getRaceSubraceOptions(rAdd.sSourceName);
	for _,v in ipairs(tRaceSubraceOptions) do
		if v.text == rAdd.sSubraceChoice then
			sSubracePath = v.linkrecord;
			break;
		end
	end
	if ((sSubracePath or "") == "") then
		CharManager.outputUserMessage("char_error_missingsubrace");
		return;
	end

	-- Notification
	CharManager.outputUserMessage("char_abilities_message_subraceadd", rAdd.sSubraceChoice, rAdd.sCharName);

	-- Update race name and subrace link
	if rAdd.sSubraceChoice:match(rAdd.sSourceName) then
		DB.setValue(rAdd.nodeChar, "race", "string", rAdd.sSubraceChoice);
	else
		DB.setValue(rAdd.nodeChar, "race", "string", string.format("%s (%s)", rAdd.sSourceName, rAdd.sSubraceChoice));
	end
	DB.setValue(rAdd.nodeChar, "subracelink", "windowreference", "reference_subrace", sSubracePath);

	-- Add racial traits
	for _,v in ipairs(DB.getChildList(DB.getPath(sSubracePath, "traits"))) do
