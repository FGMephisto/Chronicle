...
function onTabletopInit()
	...
	DB.addHandler("charsheet.*.inventorylist.*.carried", "onUpdate", CharEncumbranceManager5E.onBulkFieldUpdate);
	DB.addHandler("charsheet.*.inventorylist.*.count", "onUpdate", CharEncumbranceManager5E.onBulkFieldUpdate);
	DB.addHandler("charsheet.*.inventorylist.*.bulk", "onUpdate", CharEncumbranceManager5E.onBulkFieldUpdate);
	DB.addHandler("charsheet.*.inventorylist", "onChildDeleted", CharEncumbranceManager5E.onBulkChildDeleted);
end
...
--
--	CHRONICLE BULK SYSTEM
--

function onBulkFieldUpdate(nodeField)
	local nodeChar = DB.getChild(nodeField, "....");
	CharEncumbranceManager5E.calcItemBulk(nodeChar);
end

function onBulkChildDeleted(nodeList)
	local nodeChar = DB.getChild(nodeList, "..");
	CharEncumbranceManager5E.calcItemBulk(nodeChar);
end

function calcItemBulk(vChar)
	local nodeChar = ActorManager.getCreatureNode(vChar);
	...
	for _,vNode in ipairs(DB.getChildList(nodeChar, "inventorylist")) do
		if DB.getValue(vNode, "carried", 0) == 2 then
			local nCount = math.max(DB.getValue(vNode, "count", 1), 1);
			local nBulk = DB.getValue(vNode, "bulk", 0);
