...
function applyOngoingDamageAdjustment(nodeActor, nodeEffect, rEffectComp)
	...
	if rEffectComp.type == "REGEN" then
		...
		local rRoll = ActionDamageD20.getRoll(nil, rAction);
		if EffectManager.isGMEffect(nodeActor, nodeEffect) then
			rRoll.bSecret = true;
		end
...
function evalAbilityHelper(rActor, sEffectAbility)
	...
	local nAbility = nil;
	if sTag == "AGI" then
		nAbility = ActorManager5E.getAbilityScore(rActor, DataCommon.ability_stol[AGI]);
	elseif sTag == "ANI" then
		nAbility = ActorManager5E.getAbilityScore(rActor, DataCommon.ability_stol[ANI]);
	elseif sTag == "ATH" then
		nAbility = ActorManager5E.getAbilityScore(rActor, DataCommon.ability_stol[ATH]);
	elseif sTag == "AWA" then
		nAbility = ActorManager5E.getAbilityScore(rActor, DataCommon.ability_stol[AWA]);
	elseif sTag == "CUN" then
		nAbility = ActorManager5E.getAbilityScore(rActor, DataCommon.ability_stol[CUN]);
	elseif sTag == "DEC" then
		nAbility = ActorManager5E.getAbilityScore(rActor, DataCommon.ability_stol[DEC]);
	elseif sTag == "END" then
		nAbility = ActorManager5E.getAbilityScore(rActor, DataCommon.ability_stol[END]);
	elseif sTag == "FIG" then
		nAbility = ActorManager5E.getAbilityScore(rActor, DataCommon.ability_stol[FIG]);
	elseif sTag == "HEA" then
		nAbility = ActorManager5E.getAbilityScore(rActor, DataCommon.ability_stol[HEA]);
	elseif sTag == "LAN" then
		nAbility = ActorManager5E.getAbilityScore(rActor, DataCommon.ability_stol[LAN]);
	elseif sTag == "KNO" then
		nAbility = ActorManager5E.getAbilityScore(rActor, DataCommon.ability_stol[KNO]);
	elseif sTag == "MAR" then
		nAbility = ActorManager5E.getAbilityScore(rActor, DataCommon.ability_stol[MAR]);
	elseif sTag == "PER" then
		nAbility = ActorManager5E.getAbilityScore(rActor, DataCommon.ability_stol[PER]);
	elseif sTag == "STA" then
		nAbility = ActorManager5E.getAbilityScore(rActor, DataCommon.ability_stol[STA]);
	elseif sTag == "STE" then
		nAbility = ActorManager5E.getAbilityScore(rActor, DataCommon.ability_stol[STE]);
	elseif sTag == "SUR" then
		nAbility = ActorManager5E.getAbilityScore(rActor, DataCommon.ability_stol[SUR]);
	elseif sTag == "THI" then
		nAbility = ActorManager5E.getAbilityScore(rActor, DataCommon.ability_stol[THI]);
	elseif sTag == "WAR" then
		nAbility = ActorManager5E.getAbilityScore(rActor, DataCommon.ability_stol[WAR]);
	elseif sTag == "WIL" then
		nAbility = ActorManager5E.getAbilityScore(rActor, DataCommon.ability_stol[WIL]);
	else
