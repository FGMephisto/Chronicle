...
function getPCPowerAction(nodeAction, sSubRoll)
	...
	if rAction.type == "cast" then
		...
		if sAttackType ~= "" then
			...
			-- ToDo: Are we getting the correct DB values?
...
function performPCPowerAction(draginfo, nodeAction, sSubRoll)
	-- Debug.chat("FN: performPCPowerAction in manager_power.lua")
...
function getPCPowerCastActionText(nodeAction)
	-- Debug.chat("FN: getPCPowerCastActionText in manager_power.lua")
	local sAttack = ""
	local sSave = ""
...
function getPCPowerDamageActionText(nodeAction)
	-- Debug.chat("FN: getPCPowerDamageActionText in manager_power.lua")
	local aOutput = {}
	local rAction, rActor = PowerManager.getPCPowerAction(nodeAction)
	if rAction then
		evalAction(rActor, nodeAction.getChild("..."), rAction)
...
function getPCPowerHealActionText(nodeAction)
	-- Debug.chat("FN: getPCPowerHealActionText in manager_power.lua")
	local sHeal = ""
...
function getPowerGroupRecord(rActor, nodePower, bNPCInnate)
	-- Debug.chat("FN: getPowerGroupRecord in manager_power.lua")
...
function evalAction(rActor, nodePower, rAction)
	-- Debug.chat("FN: evalAction in manager_power.lua")
	local aPowerGroup = nil
...
function performAction(draginfo, rActor, rAction, nodePower)
	-- Debug.chat("FN: performAction in manager_power.lua")
...
function parseAttacks(sPowerName, aWords)
	-- Debug.chat("FN: parseAttacks in manager_power.lua")
	local attacks = {}
	...
	for i = 1, #aWords do
		if StringManager.isWord(aWords[i], "attack") then
			...
			if StringManager.isNumberString(aWords[nIndex+1]) and 
				...
				-- ToDo Adjust all powercalls to meet the right nomenklatur
				rAttack.nStat = tonumber(aWords[nIndex+1]) or 1
				rAttack.nSkill = tonumber(aWords[nIndex+2]) or 0
				rAttack.nMod = tonumber(aWords[nIndex+3]) or 0
				rAttack.modifier = rAttack.nMod
...
function parseDamagePhrase(aWords, i)
	-- Debug.chat("FN: parseDamagePhrase in manager_power.lua")
...
function parseDamages(sPowerName, aWords, bMagic)
	-- Debug.chat("FN: parseDamages in manager_power.lua")
	local damages = {}
...
function consolidationHelper(aMasterAbilities, aWordStats, sAbilityType, aNewAbilities)
	-- Debug.chat("FN: consolidationHelper in manager_power.lua")
...
function parsePower(sPowerName, sPowerDesc, bPC, bMagic)
	-- Debug.chat("FN: parsePower in manager_power.lua")	
...
function parseNPCPower(nodePower, bAllowSpellDataOverride)
	-- Debug.chat("FN: parseNPCPower in manager_power.lua")
