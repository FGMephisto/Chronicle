...
function collectSkillHalfProf()
	return CharWizardManager.helperCollectDataType("skillhalf");
end
...
function importCharacter(nodeChar)
	...
	for _,vSkills in pairs(DB.getChildren(nodeChar, "skilllist")) do
		if DB.getValue(vSkills, "prof", 0) > 0 then
			...
			elseif nProf == 3 then
				table.insert(tImport.skillhalf, sSkill);
			end
		end
	end
...
function requestCommit()
	if not _bRequested then
		UserManager.requestIdentity(nil, { fnResponse = CharWizardManager.requestCommitResponse, });
		_bRequested = true;
	end
end
...
function addCommitSkills(nodeChar)
	...
	for _,sSkill in pairs(CharWizardManager.collectSkillHalfProf()) do
		CharManager.addSkillHalfProficiency(nodeChar, sSkill);
	end
end
